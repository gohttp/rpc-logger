...
func (l *Logger) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case res.status >= 500:
		l.log.Error("<< %s %s %s %d (%s) in %s", r.Method, r.RequestURI, body, res.status, size, time.Since(start))
	case res.status >= 400:
		l.log.Warning("<< %s %s %s %d (%s) in %s", r.Method, r.RequestURI, body, res.status, size, time.Since(start))
	default:
		l.log.Info("<< %s %s %s %d (%s) in %s", r.Method, r.RequestURI, body, res.status, size, time.Since(start))
	}
}
